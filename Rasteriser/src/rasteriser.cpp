...
void Draw( float yaw )
{
	...
	for( int s=0; s<triangles.size(); s++ )
	{
		...
		vertices[0].position = R * triangles[s].v0;
		vertices[1].position = R * triangles[s].v1;
		vertices[2].position = R * triangles[s].v2;
		currentNormal = triangles[s].normal;
		currentReflectance = triangles[s].color;
...
void PixelShader( const Pixel& p )
{
	...
	if( p.zinv > depthBuffer[y][x] )
	{
		...
		vec3 color = ( D + indirectLightPowerPerArea ) * currentReflectance;
...
void DrawRows( const vector<Pixel>& leftPixels, const vector<Pixel>& rightPixels )
{
	...
	{
		DrawLineSDL( screen, leftPixels[s], rightPixels[s] );
	}
